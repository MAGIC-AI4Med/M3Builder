...
class AnomalySegmentationTemp_dataloader(Dataset):
    """
    Dataset for loading 3D images and associated anomaly_masks and labels, ensuring consistent output formats.
    ...
    def __getitem__(self, idx):
        ...
        image_path = sample["image_path"] # You Must Not modify this line
        mask_path = sample["label_path"] # You Must Not modify this line
        labels = sample["labels"] # You Must Not modify this line
        ...
        elif image_path.endswith('png') or image_path.endswith('jpg'): # Do not modify this line!
            img = torch.from_numpy(np.array(Image.open(image_path))).squeeze().unsqueeze(-1) # Do not modify this line!
            image_tensor = Normalization(img) # Do not modify this line!
            mask_tensor = torch.from_numpy(np.array(Image.open(mask_path))).squeeze().unsqueeze(-1) # Do not modify this line!
            assert image_tensor.shape[-1]==1 and mask_tensor.shape[-1]==1

        assert len(image_tensor.shape)==3 and image_tensor.shape[0]!=1
        # You have to return these 3 entries:
        return {
            "images": image_tensor,
            "anomalies_mask": mask_tensor,
            "labels":labels
        }
...
if __name__ == "__main__":
    ...
    datasets = AnomalySegmentationTemp_dataloader(
        "path/to/train.json"
    )
